...
void OB();
void WB();//this could be causing the tinting issues.
void color_interpolation();
//linear color correction (to fit human perception)
...
/*want to:
turn up the greens (+ general saturation w/o turning things blue)
...
void getColor() //bilinear color interpolation
{
	/*
	R G
	E B
	one pixel has one color filter ONLY due to hardware
	then let neighbors' values dictate rgb values. 
	*/
	int i,j;
	
	//R region
	//bilinear interpolation used. 
	//need edge detection so that neighbors don't get blurred
	i=0;
	for (j = 5 ; j < 3293; j=j+2){
		r[i][j] = pic[i][j];
		g[i][j] =(pic[i][j]+pic[i+1][j])/2;
		b[i][j] = ( pic[i-1][j-1] + pic[i-1][j+1] + pic[i+1][j-1] + pic[i+1][j+1] )/4;
	}
	for( i = 2; i <2460 ; i = i+2)
		for( j = 5 ; j<3293; j = j+2)
		{
			//if it's already red, it's just the red...
			r[i][j] = pic[i][j];	
			//check up and down		
			g[i][j] = pic[i+1][j];
			//check the B on each corner!
			b[i][j] = (pic[i+1][j-1] + pic[i+1][j+1] )/2;
			//underflow?? also doesn't check for i edges
			//since it's RGBE-- ignore "every 2nd row G" b/c it's actually E
	...
	for( i = 1; i <2460 ; i = i+2)
		for( j = 5 ; j<3293; j = j+2)
        {
			//if it's already green, it's just the green...
			g[i][j] = pic[i][j];		
	...
	for( j = 6 ; j<3293; j = j+2)
		{
				...
				b[i][j] = (pic[i+1][j]);
				g[i][j] = (pic[i+1][j-1] + pic[i+1][j+1] )/4;
				r[i][j] = pic[i][j+1];
...
void medianFilter(int kernelSize) {
    ...
    for (int y = 6; y < 2460; y++) {
        for (int x = 6; x < 3293; x++) {
...
//https://citeseerx.ist.psu.edu/document?repid=rep1&type=pdf&doi=f3beb91bdd820f1ba554338160016ce3c502ba80
{
	...
	for(i=0; i<2460; i++) { //Height
		for(j=0; j<3360; j++)  // Width
		{
				...
				adj = 1.0 / (1.0 + exp(-5 * (v[i][j] - 0.5)));
				s[i][j] = normSat[(int)min(s[i][j] * adj * 255, 255.0)];
		}
	}
	//convert back
...
// Correct the HSV conversion formula and ensure proper use of saturation modification
